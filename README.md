...
engineering new grad** role opens in NYC. Runs on a Fly.io machine for ~$2/month.
...
| Gate | What it does |
|---|---|
| `locOK()` | NYC, or remote **not pinned to another country**. See below |
...
### Why location takes three regexes
...
There is a `locOK` regression table in the repo history covering the 17 real
location strings this was built against, including `US-Remote; Canada-Remote`
(accept) and `Taiwan - Remote` (reject).
